/*9.4   (Duplicate Word Removal) Write a console app that inputs a sentence from the user (assume no punctuation), then determines and displays the nonduplicate words in alphabetical order.
        Treat uppercase and lowercase letters the same. [Hint: You can use string method Split with no arguments, as in sentence.Split(), to break a sentence into an array of strings
        containing the individual words. By default, Split uses spaces as delimiters.
...
class Program
{
    static void Main(string[] args)
    {
        ...
        var determineSentence =
            from e in sentence
            orderby e.ToLower()
            select e;

        foreach (var item in determineSentence)
        {
            Console.Write(item);
        }
        Console.ReadLine();
